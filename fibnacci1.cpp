...
void pushdown(int i){
	if(tr[i].lazy){
		tr[i<<1].w=(tr[i<<1].w+tr[i].lazy*(fib[tr[i<<1].r+2]-fib[tr[i<<1].l+1]+MOD+MOD))%MOD;
		tr[(i<<1)|1].w=(tr[(i<<1)|1].w+tr[i].lazy*(fib[tr[(i<<1)|1].r+2]-fib[tr[(i<<1)|1].l+1]+MOD+MOD))%MOD;
...
void pushup(int i){
	tr[i].w=(tr[i<<1].w+tr[(i<<1)|1].w+MOD)%MOD;
}
...
int query(int l,int r,int i){
	...
	return (query(l,r,i<<1)+query(l,r,(i<<1)|1)+MOD)%MOD;
}
void modify(int l,int r,int i){
	if(tr[i].l>=l&&tr[i].r<=r){
		tr[i].w=(tr[i].w+fib[tr[i].r+2]-fib[tr[i].l+1]+MOD)%MOD;
		tr[i].lazy++;
		return;
	}
	pushdown(i);
...
int main(){
	...
	for(int i=1;i<=n;i++)scanf("%I64d",&a[i]);build(1,n,1);
	int op,l,r;
	for(int i=1;i<=m;i++){
		scanf("%d%d%d",&op,&l,&r);
		switch(op){
